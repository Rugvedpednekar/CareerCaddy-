...
def export_excel(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    for job in db.query(Job).filter(Job.user_id == user.user_id).order_by(Job.created_at.desc()).all():
    ...
    for app in db.query(Application).filter(Application.user_id == user.user_id).order_by(Application.updated_at.desc()).all():
