...
def save_extracted_job(extracted: dict, warnings: list[str], dh: str, db: Session):
    ...
    job = Job(
        job_id=uid("job"), user_id=DEFAULT_USER_ID, duplicate_hash=dh, status="SCORED",
...
def extract_job(payload: JobUrlIn, db: Session = Depends(get_db)):
    ...
    job_input = JobIn(**{key: extracted.get(key) for key in JobIn.model_fields})
    return save_extracted_job(extracted, warnings, duplicate_hash(job_input), db)

@router.post("/extract-text")
def extract_text_job(payload: JobTextIn, db: Session = Depends(get_db)):
    try:
    ...
    job_input = JobIn(**{key: extracted.get(key) for key in JobIn.model_fields})
    return save_extracted_job(extracted, warnings, text_duplicate_hash(job_input), db)
